...
{
	if (region.T.C === region.aa.C)
	{
		return 'on line ' + region.T.C;
	}
	return 'on lines ' + region.T.C + ' through ' + region.aa.C;
}
...
var $author$project$Main$Frame = $elm$core$Basics$identity;
...
var $elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {ac: fragment, ad: host, al: path, ao: port_, ar: protocol, as: query};
	});
...
var $author$project$Main$Model = F3(
	function (frameCount, width, height) {
		return {B: frameCount, H: height, I: width};
	});
		...
		_List_fromArray(
			[
				...
				A3(
					...
					$author$project$Main$countToHue(model.B),
					1,
					0.5))
			]),
		_List_fromArray(
...
var $author$project$Main$render = function (model) {
	...
	var x = -(size / 2);
	var y = -(size / 2);
	var centerY = model.H / 2;
	var centerX = model.I / 2;
	return A2(
		$joakin$elm_canvas$Canvas$shapes,
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						...
						$elm$core$Basics$degrees(model.B * 0.96))
					])),
				$joakin$elm_canvas$Canvas$Settings$fill(
				A3($avh4$elm_color$Color$rgb255, 255, 105, 180))
			]),
		_List_fromArray(
			[
				A3(
	...
	function (x, y, t, cmds) {
		return A2(
			...
			function () {
				if (!t.$) {
					var image = t.a;
					return A9($joakin$elm_canvas$Canvas$Internal$CustomElementJsonApi$drawImage, 0, 0, image.I, image.H, x, y, image.I, image.H, image.aQ);
				} else {
					var sprite = t.a;
					var image = t.b;
					return A9($joakin$elm_canvas$Canvas$Internal$CustomElementJsonApi$drawImage, sprite.J, sprite.K, sprite.I, sprite.H, x, y, sprite.I, sprite.H, image.aQ);
				}
			}(),
			cmds);
	});
		...
		return A3(
			...
			F2(
				function (width, height) {
					return {H: height, aQ: target, I: width};
				}),
			A2(
	...
	function (options, attrs, entities) {
		return A3(
			...
			A2(
				...
				A2(
					$elm$core$List$cons,
					$elm$html$Html$Attributes$height(options.H),
					A2(
						$elm$core$List$cons,
						$elm$html$Html$Attributes$width(options.I),
						attrs))),
			A2(
				$elm$core$List$cons,
				_Utils_Tuple2('__canvas', $joakin$elm_canvas$Canvas$cnvs),
				A2($elm$core$List$map, $joakin$elm_canvas$Canvas$renderTextureSource, options.az)));
	});
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_Utils_Tuple2(
					$elm$core$Basics$floor(model.I),
					$elm$core$Basics$floor(model.H)),
				_List_Nil,
				_List_fromArray(
					[
...
var $author$project$Main$main = $elm$browser$Browser$element(
	{
		...
		a0: function (model) {
			return A2($elm$time$Time$every, 40, $elm$core$Basics$identity);
		},
